...
export type Work = {
  ...
  const data: Work[] = [
    ...
    },
    {
      id: "2",
      title: "スマブラ風ゲームAI",
      description: "簡単なスマブラ風ゲームを作成し，強化学習(PPO)を用いてゲームAIを学習した．ゲームはとても簡単な仕様であり，移動(左右とジャンプ)と攻撃のみ対応している．",
      motivation: "サッカーAIを作成するための学習材料として，簡単なゲームで強化学習を学びたかったため．",
      implementation: "ゲームはPygameを用いて作成した．エージェントは環境から11つの変数(距離_x,距離_y,p1_x,p1_y,p2_x,p2_x,攻撃範囲かどうか，p1_硬直時間，p2_硬直時間,p1_ダメージ量,p2_ダメージ量)を受け取る．エージェントはこれを元に「攻撃するorしない」と「移動方向(左右上)か移動しない」を選択する．エージェントが環境から受け取る即時報酬は「攻撃する(+4),攻撃される(-3),勝利(+10),敗北(-10),距離xに応じてマイナス(-dist_x / 500),終了していないならマイナス(-0.01)」である．",
      difficulty: "まだPPOの細かいアルゴリズムを理解しきれていない．Actor-Criticまでは理解することができた．また，報酬設計がとても難しい．",
